...
def test_blocked_user_can_delete_own_event(client, app):
    ...
    app.config['WTF_CSRF_ENABLED'] = False
    with app.app_context():
        user = User(email='blocked_host@example.com', password_hash=bcrypt.generate_password_hash('password').decode('utf-8'),
...
def test_admin_anonymous_event_profile_privacy(client, app):
    ...
    # User tries to view admin profile (should be 404 as per existing logic, 
    # but let's check if the event is there if it wasn't 404)
    # Actually admin profile is 404 for regular users.
    response = client.get(f'/profile/{admin_id}')
    assert response.status_code == 404
